...
lm_prelim <- lm(Salary ~ RBI + CHmRun + CRBI + League + Division + 
                PutOuts + Assists + Errors + NewLeague, data = hitters_train)
summary(lm_prelim)
vif(lm_prelim)
...
boxCox(lm_prelim)
hitters_train$trans_Salary <- hitters_train$Salary^(1/4)
...
lm_int <- lm(trans_Salary ~ RBI + CHmRun + CRBI + League + Division + 
               PutOuts + Assists + Errors + NewLeague + RBI*CHmRun + RBI*CRBI + RBI*League +
               RBI*Division + RBI*PutOuts + RBI*Assists + RBI*Errors + RBI*NewLeague +
               CHmRun*CRBI + CHmRun*League + CHmRun*Division + CHmRun*PutOuts +
               CHmRun*Assists + CHmRun*Errors + CHmRun*NewLeague + CRBI*League +
               CRBI*Division + CRBI*PutOuts + CRBI*Assists + CRBI*Errors + CRBI*NewLeague +
               League*Division + League*PutOuts + League*Assists + League*Errors +
               League*NewLeague + Division*PutOuts + Division*Assists + Division*Errors +
               Division*NewLeague + PutOuts*Assists + PutOuts*Errors + PutOuts*NewLeague +
               Assists*Errors + Assists*NewLeague + Errors*NewLeague +
               I(HmRun^2) + I(RBI^2) + I(CHmRun^2) + I(CRBI^2) +
...
# Backward model performs the best, check modeling assumptions
# Constant variance
...
hitters_test$trans_Salary <- hitters_test$Salary^(1/4)
test_fit <- predict(lm_backward, newdata = hitters_test)
test_fit_adj <- test_fit^(4)
MAE <- mean(abs(hitters_test$Salary - test_fit_adj))
sd(hitters_train$Salary)
# MAE returns a value of 226.83, indicating a good model because the standard deviation
# of the training dataset is 434.26. Salary is reported in thousands of dollars, meaning
# that model predictions were off on average by $226,830. Although this data is from 1986,
# the same variables are likely to be important in modeling salary today, meaning that career 
# home runs and career/yearly RBI's are significant offensive metrics, as well as the 
# defensive metrics of assists and errors. Division location was also important, and the rest
# of the variables included in final models indicate meaningful interactions among variables.
